  ...
  return (
    <svg width={size} height={size} viewBox={`0 0 ${vw} ${vh}`} className="radar-chart">
      ...
      {gridPaths.map((points, i) => (
        <polygon key={i} points={points} fill="none" stroke="#F8BBD0" strokeWidth="1" opacity={0.5 + i * 0.1} />
      ))}

      {models.map((_, i) => {
        const p = getPoint(i, 100);
        return <line key={i} x1={cx} y1={cy} x2={p.x} y2={p.y} stroke="#F8BBD0" strokeWidth="1" opacity={0.3} />;
      })}

      <polygon points={dataPath} fill="url(#radarFill)" stroke="#E91E63" strokeWidth="2.5" />
