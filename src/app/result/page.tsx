...
import CharacterEmoji from "@/components/CharacterEmoji";
...
export default function ResultPage() {
  ...
  useEffect(() => {
    try {
      const raw = sessionStorage.getItem("lbti_data");
  ...
  const generatePoster = useCallback(async () => {
    ...
    bar.addColorStop(0, "#E91E63");
    bar.addColorStop(1, "#F06292");
    ctx.fillStyle = bar;
    ...
    ctx.fillText("LBTI · 恋爱行为类型测试", W / 2, f(55));

    // Emoji character
    ctx.font = `${f(180)}px system-ui, sans-serif`;
    ctx.fillText(p.emoji, W / 2, f(310));

    // Code
    ctx.fillStyle = p.color;
    ctx.font = `900 ${f(88)}px ui-monospace, monospace`;
    ctx.fillText(p.code, W / 2, f(440));

    // Name
    ctx.fillStyle = "#1c1917";
    ctx.font = `900 ${f(44)}px system-ui, sans-serif`;
    ctx.fillText(p.name, W / 2, f(500));

    // Motto
    ctx.fillStyle = "#78716c";
    ctx.font = `${f(22)}px system-ui, sans-serif`;
    ctx.fillText(`「${p.motto}」`, W / 2, f(540));
    ...
    ctx.fillStyle = "#1c1917";
    ctx.font = `900 ${f(28)}px system-ui, sans-serif`;
    ctx.fillText("你是什么类型的恋人？", ctaX, bottomY + f(50));

    ctx.fillStyle = "#E91E63";
    ctx.font = `900 ${f(22)}px system-ui, sans-serif`;
    ctx.fillText("扫码测测你的恋爱人格", ctaX, bottomY + f(90));

    ctx.fillStyle = "#a8a29e";
    ctx.font = `${f(18)}px system-ui, sans-serif`;
    ctx.fillText("30 道题 · 27 种恋爱人格", ctaX, bottomY + f(130));

    // Footer
    ctx.textAlign = "center";
    ctx.fillStyle = "#d6d3d1";
    ctx.font = `${f(16)}px system-ui, sans-serif`;
    ctx.fillText("LBTI · 恋爱行为类型测试", W / 2, H - f(30));
  ...
  if (!result) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-b from-[#FFF5F8] to-[#FFF0F5]">
        <div className="text-center">
          <div className="animate-spin rounded-full h-12 w-12 border-4 border-pink-400 border-t-transparent mx-auto mb-4" />
          <p className="text-stone-400 text-sm">正在分析你的恋爱人格...</p>
        </div>
      </div>
    );
  }
  ...
  const siteUrl = getSiteUrl();
  const shareText = `我在 LBTI 恋爱人格测试中测出了【${p.code} · ${p.name}】！\n「${p.motto}」\n匹配度 ${similarity}%\n\n你是 WIFI 还是 GHOST？来测测 👉 ${siteUrl}`;
  ...
  const handleSavePoster = () => {
    ...
    a.href = posterUrl;
    a.download = `LBTI-${p.code}.png`;
    a.click();
  };

  const models = ["A", "E", "P", "F", "L"];

  return (
    <main className="min-h-screen bg-gradient-to-b from-[#FFF5F8] to-[#FFF0F5]">
      <div className="grain-overlay" />

      <header className="sticky top-0 z-50 bg-white/80 backdrop-blur-lg border-b border-pink-100/50">
        <div className="max-w-2xl mx-auto px-4 py-3 flex items-center justify-between">
          <button onClick={() => router.push("/")} className="text-stone-400 hover:text-pink-500 transition text-sm font-medium">← 首页</button>
          <span className="font-mono font-black text-pink-500 text-sm">LBTI</span>
      ...
      <div className="max-w-2xl mx-auto px-4 py-8">
        <div className="slide-up">
          {/* Main Card */}
          <div className="rounded-3xl overflow-hidden shadow-2xl mb-8 border border-pink-100" style={{ background: `linear-gradient(160deg, ${p.color}08, ${p.color}18, ${p.color}08)` }}>
            <div className="p-10 text-center relative">
              ...
              {result.isSpecial && (
                <div className="inline-block bg-rose-100 text-rose-700 text-xs font-black px-4 py-1.5 rounded-lg mb-4">🔄 隐藏人格触发！</div>
              )}
              <div className="float-animation mb-6">
                <CharacterEmoji type={p.code} size={170} className="mx-auto" />
              </div>
              ...
              <div className="inline-flex items-center gap-2 bg-white/70 backdrop-blur px-5 py-2.5 rounded-full border border-pink-100">
                <span className="text-xs text-stone-400">匹配度</span>
                <span className="text-3xl font-black" style={{ color: p.color }}>{similarity}%</span>
              </div>
            </div>
            <div className="bg-white/60 backdrop-blur px-8 py-6 border-t border-pink-50">
              <p className="text-stone-600 leading-relaxed text-sm">{p.description}</p>
            </div>
          </div>

          {/* Radar */}
          <div className="bg-white rounded-2xl shadow-lg p-6 mb-5 border border-pink-50">
              ...
              <div className="space-y-2.5">
                {matchDetails.map((m, i) => (
                  <div key={m.code} className="flex items-center gap-3">
                    <span className={`text-xs font-black w-6 h-6 rounded-lg flex items-center justify-center ${i === 0 ? "bg-pink-100 text-pink-600" : "bg-stone-100 text-stone-400"}`}>{i + 1}</span>
                    ...
                    <div className="w-20 h-2 bg-pink-100 rounded-full overflow-hidden">
                      <div className="h-full rounded-full bg-gradient-to-r from-pink-400 to-rose-400" style={{ width: `${m.similarity}%` }} />
                    </div>
                  ...
                  return (
                    <div key={m}>
                      ...
                      <div className="space-y-3 pl-1">
                        {modelDims.map((dim) => (
                          <div key={dim.code} className="border-l-2 border-pink-100 pl-4">
                            <div className="flex items-center justify-between mb-1">
                              <span className="text-xs font-medium text-stone-600">{dim.code} {dim.name}</span>
                              <span className={`text-[10px] font-black px-2 py-0.5 rounded-md ${dim.level === "H" ? "bg-pink-100 text-pink-600" : dim.level === "M" ? "bg-rose-50 text-rose-600" : "bg-stone-100 text-stone-400"}`}>{dim.level}</span>
                            </div>
                            <div className="h-1.5 bg-pink-50 rounded-full overflow-hidden">
                              <div className="h-full rounded-full transition-all duration-1000 bg-gradient-to-r from-pink-400 to-rose-400" style={{ width: `${dim.percentage}%` }} />
                            </div>
                          </div>
                        ))}
                      </div>
          ...
          <div className="grid grid-cols-2 gap-4 mb-5">
            <div className="bg-white rounded-2xl shadow-lg p-5 border border-pink-50">
              <h4 className="font-black text-sm mb-3 text-pink-600">💪 优势</h4>
              <ul className="space-y-2">{p.strengths.map((s, i) => <li key={i} className="text-xs text-stone-600 flex items-start gap-2"><span className="text-pink-400 mt-0.5 font-bold">✓</span>{s}</li>)}</ul>
            </div>
            <div className="bg-white rounded-2xl shadow-lg p-5 border border-pink-50">
              <h4 className="font-black text-sm mb-3 text-rose-600">⚠️ 注意</h4>
              <ul className="space-y-2">{p.weaknesses.map((w, i) => <li key={i} className="text-xs text-stone-600 flex items-start gap-2"><span className="text-rose-400 mt-0.5 font-bold">!</span>{w}</li>)}</ul>
            </div>
          </div>

          {/* Love Style & Spirit */}
          <div className="bg-white rounded-2xl shadow-lg p-6 mb-5 border border-pink-50">
            <div className="mb-4"><h4 className="font-black text-xs text-stone-400 uppercase tracking-wider mb-1">💘 恋爱风格</h4><p className="text-sm text-stone-700">{p.loveStyle}</p></div>
          ...
          <div className="share-card rounded-2xl shadow-2xl p-6 text-center text-white mb-5 relative overflow-hidden">
            ...
            <div className="relative z-10">
              <h3 className="font-black text-lg mb-2">分享你的恋爱人格</h3>
              <p className="text-white/60 text-xs mb-4">让更多人发现自己的恋爱类型</p>
              ...
              <div className="flex gap-3 justify-center flex-wrap">
                <button onClick={handleCopy} className="bg-white text-pink-600 px-5 py-2.5 rounded-full font-black text-sm hover:bg-pink-50 transition-all active:scale-95 shadow-lg">
                  {copied ? "✓ 已复制！" : "📋 复制文案"}
                </button>
          ...
          <div className="flex gap-3">
            <button onClick={() => { sessionStorage.removeItem("lbti_data"); router.push("/test"); }}
              className="flex-1 bg-white text-pink-500 border-2 border-pink-300 px-6 py-3 rounded-full font-black text-sm hover:bg-pink-50 transition-all">重新测试</button>
            <button onClick={() => router.push("/")}
              className="flex-1 bg-pink-500 text-white px-6 py-3 rounded-full font-black text-sm hover:bg-pink-600 transition-all shadow-lg">回到首页</button>
          </div>

          <div className="mt-10 text-center text-[10px] text-stone-300 pb-8 space-y-1">
            <p className="font-mono">LBTI · 恋爱行为类型测试 · 仅供娱乐</p>
          </div>
        </div>
      </div>
    </main>
